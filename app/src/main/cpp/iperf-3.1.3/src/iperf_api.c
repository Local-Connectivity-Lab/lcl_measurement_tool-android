...
iperf_on_connect(struct iperf_test *test)
{
    ...
    if (test->role == 'c') {
        if (test->json_output)
            ...
            // jni callback
            if (test->jniCallback)
                test->jniCallback->on_connecting(test, test->server_hostname, test->server_port);
    ...
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
		...
		client_flag = 1;
            ...
            case OPT_NO_FQ_SOCKET_PACING:
                ...
                break;
            case OPT_TMP_PATH:
                test->tmp_template = strdup(optarg);
                break;
            case 'h':
            default:
                usage_long();
...
connect_msg(struct iperf_stream *sp)
{
    ...
    if (sp->test->json_output)
        ...
        if (test->jniCallback)
            test->jniCallback->on_connected(test, ipl, lport, ipr, rport);
...
iperf_free_test(struct iperf_test *test)
{
    ...
    if(test->jniCallback) {
        free(test->jniCallback);
    }
    test->jniCallback = NULL;
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->sender && test->sender_has_retransmits) {
                    ...
                    if (test->json_output)
                        ...
                        // jni callback
                        if (test->jniCallback)
                            test->jniCallback->on_interval(test, (float) start_time, (float) end_time, ubuf, nbuf);
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->sender_has_retransmits) {
                    ...
                    if (test->json_output)
                        ...
                        // TODO 2020/11/24: 以本地计算结果还是以服务器端结果为准？用哪个值回调给Java？
                        // 只有上行测试时，发送数据计算结果才表示本地带宽。而下行测试时，这个发送数据计算结果表示远端(sender)
                        // 的带宽。因为我们只向Java回调通知本地带宽结算结果，所以只在上行测试时回调。
                        // jni callback
                        if (test->jniCallback && test->num_streams == 1)
                            test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
                    ...
                    if (test->json_output)
                        ...
                        // jni callback。只回调本地端带宽，原因同上。
                        if (!test->reverse && test->jniCallback && test->num_streams == 1) {
                            test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
                        }
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->json_output)
                    ...
                    // 只有下行测试时，接收数据计算结果才表示本地带宽。而上行测试时，这个接收数据计算结果表示远端(receiver)
                    // 的带宽。因为我们只向Java回调通知本地带宽结算结果，所以只在下行测试时回调。
                    // jni callback
                    if (test->reverse && test->jniCallback && test->num_streams == 1) {
                        test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
                    }
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender_has_retransmits) {
                ...
                if (test->json_output)
                    ...
                    // 此时的bandwidth由total_sent计算得到，所以只有是上行模式时，才表示本地的带宽；下行模式时发送数据计算
                    // 结果则表示远端的带宽。因为我们只向Java回调通知本地带宽结算结果，所以只在上行测试时回调。
                    // jni callback
                    if (!test->reverse && test->jniCallback)
                        test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
            ...
            if (test->json_output)
                ...
                // 此时的bandwidth由total_received计算得到，所以只有下行模式时，才表示本地的带宽；上行模式时接收数据
                // 计算结果则表示远端的带宽。因为我们只向Java回调通知本地带宽结算结果，所以只在下行测试时回调。
                // jni callback
                if (test->reverse && test->jniCallback)
                    test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
            ...
            if (test->json_output)
                ...
                // 此时的bandwidth由total_send计算得到，所以只有上行模式时，才表示本地的带宽；下行模式时发送数据
                // 计算结果则表示远端的带宽。因为我们只向Java回调通知本地带宽结算结果，所以只在上行测试时回调。
                // jni callback
                if (!test->reverse && test->jniCallback)
                    test->jniCallback->on_result(test, (float) start_time, (float) end_time, ubuf, nbuf);
    ...
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        if (test->sender && test->sender_has_retransmits) {
            ...
            if (test->json_output)
                ...
                // jni callback
                if (test->jniCallback && test->num_streams == 1)
                    test->jniCallback->on_interval(test, (float) st, (float) et, ubuf, nbuf);
...
//        char buf[] = "/tmp/iperf3.XXXXXX";
        char buf[] = "/data/data/com.lcl.lclmeasurementtool/cache/iperf3.XXXXXX";
